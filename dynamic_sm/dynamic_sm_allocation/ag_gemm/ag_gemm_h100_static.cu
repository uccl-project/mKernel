...
__device__ __forceinline__ unsigned long long ag_gemm_global_time_ns()
{
...
namespace ag_gemm_static
{
    ...
    struct globals
    {
        ...
        unsigned long long *timing_buf; // device 0 only: [0]=comp_total_ns, [1]=comp_count
#ifdef AG_GEMM_PRINT_SCHED_STATS
        unsigned long long *spin_cycles_total;
        unsigned long long *spin_trace_buf;
        unsigned long long *spin_trace_time_buf;
        unsigned int *spin_trace_idx;
        static constexpr unsigned int SPIN_TRACE_MAX = 8192;
#endif
...
#ifdef AG_GEMM_PRINT_SCHED_STATS
    static unsigned long long *g_spin_cycles_total[globals::NUM_DEVICES] = {nullptr};
    static unsigned long long *g_spin_trace_buf[globals::NUM_DEVICES] = {nullptr};
    static unsigned long long *g_spin_trace_time_buf[globals::NUM_DEVICES] = {nullptr};
    static unsigned int *g_spin_trace_idx[globals::NUM_DEVICES] = {nullptr};
#endif
            ...
            if (warp_id == 0 && lane_id == 0)
            {
                for (int task_id = blockIdx.x; task_id < num_global_blocks; task_id += G.num_comp_sms)
                {
...
#ifdef AG_GEMM_PRINT_SCHED_STATS
                        if (G.spin_cycles_total != nullptr &&
                            G.spin_trace_buf != nullptr &&
                            G.spin_trace_time_buf != nullptr &&
                            G.spin_trace_idx != nullptr)
                        {
                            const unsigned long long wait_t0 = clock64();
                            wait(G.barrier, {row_idx / 2}, G.dev_idx, num_comm_workers_per_stage);
                            const unsigned long long wait_cycles = (clock64() - wait_t0) >> 10;
                            const unsigned long long cumulative =
                                atomicAdd((unsigned long long *)G.spin_cycles_total, wait_cycles) +
                                wait_cycles;
                            const unsigned int slot =
                                atomicAdd((unsigned int *)G.spin_trace_idx, 1u);
                            if (slot < globals::SPIN_TRACE_MAX)
                            {
                                G.spin_trace_buf[slot] = cumulative;
                                G.spin_trace_time_buf[slot] = ag_gemm_global_time_ns();
                            }
                        }
                        else
#endif
                    ...
                    for (int red_idx = 0; red_idx < num_iters; red_idx++)
                    {
                        ...
                        { // ASSUMPTION: K is always a multiple of 4*64
...
#pragma unroll
        ...
        else
        {
            ...
            for (int task_id = blockIdx.x; task_id < num_global_blocks; task_id += G.num_comp_sms)
            {
                unsigned long long comp_t0 = 0;
                // Time compute-task latency from consumer side (includes wait on inputs + MMA + output arrive).
                if (G.timing_buf != nullptr && warpgroup_id == 0 && warp_id == 0 && lane_id == 0)
                    comp_t0 = ag_gemm_global_time_ns();
                ...
                if (G.timing_buf != nullptr && warpgroup_id == 0 && warp_id == 0 && lane_id == 0)
                {
                    atomicAdd((unsigned long long *)G.timing_buf, ag_gemm_global_time_ns() - comp_t0);
                    atomicAdd((unsigned long long *)G.timing_buf + 1, 1ULL);
                }
            }
        }
    }
        ...
        const int active_sms = config::NUM_BLOCKS)
    {
        ...
        TORCH_CHECK(num_comm_sms < active_sms,
                    "num_comm_sms (", num_comm_sms, ") must be < active_sms (", active_sms, ")");
        const int dev_idx = barrier.local_rank_;
        c10::cuda::CUDAGuard device_guard(dev_idx);
#ifdef AG_GEMM_PRINT_SCHED_STATS
        cudaStream_t stream = at::cuda::getCurrentCUDAStream(dev_idx).stream();
        if (g_spin_cycles_total[dev_idx] == nullptr)
            cudaMalloc(&g_spin_cycles_total[dev_idx], sizeof(unsigned long long));
        if (g_spin_trace_buf[dev_idx] == nullptr)
            cudaMalloc(&g_spin_trace_buf[dev_idx],
                       globals::SPIN_TRACE_MAX * sizeof(unsigned long long));
        if (g_spin_trace_time_buf[dev_idx] == nullptr)
            cudaMalloc(&g_spin_trace_time_buf[dev_idx],
                       globals::SPIN_TRACE_MAX * sizeof(unsigned long long));
        if (g_spin_trace_idx[dev_idx] == nullptr)
            cudaMalloc(&g_spin_trace_idx[dev_idx], sizeof(unsigned int));
        cudaMemsetAsync(g_spin_cycles_total[dev_idx], 0, sizeof(unsigned long long), stream);
        cudaMemsetAsync(g_spin_trace_buf[dev_idx], 0,
                        globals::SPIN_TRACE_MAX * sizeof(unsigned long long), stream);
        cudaMemsetAsync(g_spin_trace_time_buf[dev_idx], 0,
                        globals::SPIN_TRACE_MAX * sizeof(unsigned long long), stream);
        cudaMemsetAsync(g_spin_trace_idx[dev_idx], 0, sizeof(unsigned int), stream);
#endif
        ...
        globals G{
            ...
            .num_comp_sms = active_sms - num_comm_sms,
            .timing_buf = nullptr
#ifdef AG_GEMM_PRINT_SCHED_STATS
            ,
            .spin_cycles_total = g_spin_cycles_total[dev_idx],
            .spin_trace_buf = g_spin_trace_buf[dev_idx],
            .spin_trace_time_buf = g_spin_trace_time_buf[dev_idx],
            .spin_trace_idx = g_spin_trace_idx[dev_idx]
#endif
        };
        kittens::py::launch_kernel<config, globals, main_kernel>(G, (unsigned int)active_sms);
        kittens::py::launch_kernel<config, globals, epilogue_kernel>(G);
    }

#ifdef AG_GEMM_PRINT_SCHED_STATS
    at::Tensor get_spin_trace_buf_py(int dev_idx)
    {
        unsigned int tidx = 0;
        if (g_spin_trace_idx[dev_idx] != nullptr)
            cudaMemcpy(&tidx, g_spin_trace_idx[dev_idx], sizeof(unsigned int),
                       cudaMemcpyDeviceToHost);
        int n = (int)min(tidx, globals::SPIN_TRACE_MAX);
        auto out = at::zeros({n}, at::TensorOptions().dtype(at::kLong));
        if (n > 0 && g_spin_trace_buf[dev_idx] != nullptr)
            cudaMemcpy(out.data_ptr<int64_t>(), g_spin_trace_buf[dev_idx],
                       (size_t)n * sizeof(unsigned long long), cudaMemcpyDeviceToHost);
        return out;
    }

    at::Tensor get_spin_trace_time_buf_py(int dev_idx)
    {
        unsigned int tidx = 0;
        if (g_spin_trace_idx[dev_idx] != nullptr)
            cudaMemcpy(&tidx, g_spin_trace_idx[dev_idx], sizeof(unsigned int),
        ...
        int n = (int)min(tidx, globals::SPIN_TRACE_MAX);
        auto out = at::zeros({n}, at::TensorOptions().dtype(at::kLong));
        if (n > 0 && g_spin_trace_time_buf[dev_idx] != nullptr)
            cudaMemcpy(out.data_ptr<int64_t>(), g_spin_trace_time_buf[dev_idx],
                       (size_t)n * sizeof(unsigned long long), cudaMemcpyDeviceToHost);
        return out;
    }
#endif
