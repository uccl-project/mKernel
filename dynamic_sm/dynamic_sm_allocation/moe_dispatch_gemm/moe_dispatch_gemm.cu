...
// Model shape constants — override at compile time with -D flags as needed.
...
struct globals {
    ...
    // NOTE: This "barrier" is only used for intra-device synchronization of
    // dispatch progress (we only ever read/write barrier[G.dev_idx][...]).
    // It does NOT require multicast/VMM.
...
// Variant that overrides the effective SM count (for reduced-CTA experiments).
void entrypoint_with_active_sms(
    kittens::py::TKParallelTensor &pre_tokens,
    at::Tensor &post_tokens,
    at::Tensor &weights,
    at::Tensor &outputs,
    ...
    const int eff_sm = (active_sms > 0 && active_sms < SM_COUNT) ? active_sms : SM_COUNT;
    globals G {
        .pre_tokens=kittens::py::parallel_tensor_to_pgl<globals::pre_tokens_pgl>(pre_tokens),
        .post_tokens=kittens::py::tensor_to_gl<globals::post_tokens_gl>(post_tokens),
        .weights=kittens::py::tensor_to_gl<globals::weights_gl>(weights),
        ...
        .num_padded_local_tokens = num_padded_local_tokens,
        .num_comm_sms = num_comm_sms,
        .num_comp_sms = eff_sm - num_comm_sms
    };
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("moe_dispatch_gemm_active_sms", &entrypoint_with_active_sms);
}
