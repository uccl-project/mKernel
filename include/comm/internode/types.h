...
struct TransferCmd {
    ...
    uint8_t  dst_rank;       // target node rank
...
// Cap on inter-node peer slots per session (== num_nodes - 1).
...
//
// For N == 2, slot_at_peer is always 0, regardless of (my_rank, peer_rank).
// The expressions
//   cmd.remote_offset = slot_at_peer * single_peer_bytes + base_offset
//   cmd.tile_id      = slot_at_peer * single_peer_tiles + chunk_id
// therefore add zero offsets at N == 2. For N > 2 the offsets partition the receiver's
