...
// Minimal CTA-group 1 tcgen05 validation path. B is supplied in the native
// shared-memory orientation [K, N], matching the model weight layout.
struct tcgen05_micro_globals {
    using A_tile = st_bf<128, 64>;
    using B_tile = st_bf<64, 256>;
    using C_tile = st_bf<128, 32>;

    using A_tensor = dist::local_tensor<bf16, 1, 1, 128, 64, A_tile>;
    using B_tensor = dist::local_tensor<bf16, 1, 1, 64, 256, B_tile>;
    using C_tensor = dist::local_tensor<bf16, 1, 1, 128, 256, C_tile>;
...
inline void tcgen05_micro(at::Tensor &A,
                          at::Tensor &B,
                          at::Tensor &C) {
    c10::cuda::CUDAGuard device_guard(A.device());
    cudaStream_t stream = at::cuda::getCurrentCUDAStream().stream();

    TORCH_CHECK(A.sizes() == at::IntArrayRef({128, 64}) &&
                A.scalar_type() == at::kBFloat16 && A.is_contiguous(),
                "A must be contiguous bf16 [128, 64]");
    TORCH_CHECK(B.sizes() == at::IntArrayRef({64, 256}) &&
                B.scalar_type() == at::kBFloat16 && B.is_contiguous(),
                "B must be contiguous bf16 [64, 256]");
    TORCH_CHECK(C.sizes() == at::IntArrayRef({128, 256}) &&
                C.scalar_type() == at::kBFloat16 && C.is_contiguous(),
                "C must be contiguous bf16 [128, 256]");
    TORCH_CHECK(A.device() == B.device() && A.device() == C.device(),
                "A, B, and C must be on the same GPU");

    tcgen05_micro_globals G{
        .A = ::dist::local_tensor_from_tensor<
            tcgen05_micro_globals::A_tensor>(A),
        .B = ::dist::local_tensor_from_tensor<
            tcgen05_micro_globals::B_tensor>(B),
        .C = ::dist::local_tensor_from_tensor<
            tcgen05_micro_globals::C_tensor>(C),
    };
    launch_tcgen05_micro(G, stream);
}
